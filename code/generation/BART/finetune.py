...
def main():
    ...
    if training_args.do_predict:
        ...
        eval_metrics = {k: v for k, v in eval_output.metrics.items()}
        ...
        if trainer.is_world_process_zero():
            ...
            save_json(eval_metrics, os.path.join(training_args.output_dir, "eval_results.json"))
            eval_results.update(eval_metrics)

            if training_args.predict_with_generate:
                eval_preds = tokenizer.batch_decode(
                    eval_output.predictions, skip_special_tokens=True, clean_up_tokenization_spaces=True
                )
                eval_preds = lmap(str.strip, eval_preds)
                write_txt_file(eval_preds, os.path.join(training_args.output_dir, "eval_generations.txt"))
