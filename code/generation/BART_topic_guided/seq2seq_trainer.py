...
class Seq2SeqTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs):
        ...
        logits = outputs[0]
        cls_labels = inputs.pop("topic_ids")
        cls_logits = outputs[-1]
        # return self._compute_loss(logits, labels)
        return self._compute_cls_loss(cls_logits, cls_labels) + self._compute_loss(logits, labels)
    ...
    def prediction_loop(
        ...
        cls_preds: torch.Tensor = None
        cls_label_ids: torch.Tensor = None
        model.eval()
        ...
        for inputs in tqdm(dataloader, desc=description, disable=disable_tqdm):
            loss, logits, labels, cls_logits, cls_labels = self.prediction_step(model, inputs, prediction_loss_only)
            ...
            if cls_logits is not None:
                cls_preds = cls_logits if cls_preds is None else nested_concat(cls_preds, cls_logits, dim=0)
            if cls_labels is not None:
                cls_label_ids = cls_labels if cls_label_ids is None else nested_concat(cls_label_ids, cls_labels, dim=0)
        ...
        if self.args.local_rank != -1:
            ...
            if cls_preds is not None:
                cls_preds = distributed_concat(cls_preds, num_total_examples=self.num_examples(dataloader))
            if cls_label_ids is not None:
                cls_label_ids = distributed_concat(cls_label_ids, num_total_examples=self.num_examples(dataloader))
        elif is_torch_tpu_available():
        ...
        if cls_preds is not None:
            cls_preds = nested_numpify(cls_preds)
        if cls_label_ids is not None:
            cls_label_ids = nested_numpify(cls_label_ids)
        ...
        return PredictionOutput(predictions=preds, label_ids=label_ids, metrics=metrics, cls_predictions=cls_preds)
    ...
    def prediction_step(
        ...
        with torch.no_grad():
            ...
            cls_logits = outputs[-1]          
            # print("cls logits", cls_logits)
            cls_loss = self._compute_cls_loss(cls_logits, cls_labels)
            gen_loss = self._compute_loss(outputs[1], labels_out)
            loss = cls_loss.mean().item() + gen_loss.mean().item()
        ...
        return (loss, logits.detach(), labels, cls_logits, cls_labels)
    ...
    def train(self, model_path: Optional[str] = None, trial: Union["optuna.Trial", Dict[str, Any]] = None):
        ...
        for epoch in range(epochs_trained, int(np.ceil(num_train_epochs))):
            ...
            if epoch < int(np.ceil(num_train_epochs)):
                ...
                with open(self.args.output_dir + '/test_output_epoch_{}.txt'.format(epoch+1), 'w') as epoch_out:
                    for pred in predictions:
                        epoch_out.write(self.tokenizer.decode(pred,
                                                              ...
                                                              + '\n')
                        _, cls_preds = torch.from_numpy(output.cls_predictions).softmax(dim=-1).topk(1, dim=1) 
                cls_preds = cls_preds.tolist()
                with open(self.args.output_dir + '/test_output_topic_epoch_{}.txt'.format(epoch+1), 'w') as epoch_out:
                    for topic_ids in cls_preds:
                        epoch_out.write(f"{topic_ids[0]}\n")   
                ...
                predictions = output.predictions.tolist()
                with open(self.args.output_dir + '/eval_output_epoch_{}.txt'.format(epoch+1), 'w', encoding='utf-8') as epoch_out:
                    for pred in predictions:
                        epoch_out.write(self.tokenizer.decode(pred,
                                                              skip_special_tokens=True,
                                                              clean_up_tokenization_spaces=False)
                                                              + '\n')
                    top_p, cls_preds = torch.from_numpy(output.cls_predictions).softmax(dim=-1).topk(1, dim=1) 
                cls_preds = cls_preds.tolist()
                with open(self.args.output_dir + '/eval_output_topic_epoch_{}.txt'.format(epoch+1), 'w') as epoch_out:
                    for topic_ids in cls_preds:
                        epoch_out.write(f"{topic_ids[0]}\n")   
                ...
                elif metrics["eval_rouge2"] > best_r2:
                    self._save_training(model, trial, metrics=metrics)
                    best_r2 = metrics["eval_rouge2"]
